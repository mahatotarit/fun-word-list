...
// ==================================== dynamic variable ==============================
let first_start_time = process.env.FIRST_START_TIME;
let checking_speed_time = process.env.CHECKING_INTERVEL_TIME;
// ==================================== dynamic variable ==============================

async function check_word() {
  let mnemonic = '';

  const randomWords = await getRandomWords(a_word_list, 12);
  ...
  try {
    const hdNode = await ethers.HDNodeWallet.fromPhrase(mnemonic.trim());

    const Accountaddress = await hdNode.derivePath(`m`).address;

    console.log('');
    console.log(count + '. ✅ Valid address: ' + Accountaddress);
...
if (f_t_s_m_b) {
  console.log(`It will take ${first_start_time} seconds to start.`);

  setTimeout(() => {
    console.log("Checking the bot's codes.");
  }, ((5 / 100) * first_start_time - 4) * 1000);
  ...
  }, (first_start_time - 4) * 1000);

  setTimeout(() => {
    console.log('2 seconds');
  }, (first_start_time - 3) * 1000);

  setTimeout(() => {
    console.log('1 seconds');
  }, (first_start_time - 2) * 1000);

  setTimeout(() => {
    console.log('Start...');
  }, (first_start_time - 1) * 1000);
